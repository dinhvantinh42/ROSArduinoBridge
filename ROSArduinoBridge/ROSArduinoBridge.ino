...
int runCommand() {
  ...
  if (arg1 != arg2) {
      ...
      moving = 1;   
      if (arg1 == arg2) {  // Đi thẳng
      long current_left = readEncoder(LEFT);
      long current_right = readEncoder(RIGHT);
      long delta_left = current_left - en_begin_left;
      long delta_right = current_right - en_begin_right;
      long error = delta_left - delta_right;
      
      // Sử dụng PID để điều chỉnh tốc độ
      float correction = computePID(error);
      
      int left_speed = arg1 - correction;
      // int right_speed = arg1 + correction;
      
      // Giới hạn tốc độ trong phạm vi cho phép
    ...
    } else {  // Rẽ trái hoặc phải
      setMotorSpeeds(arg1, arg2);
    }
    
    // Cập nhật giá trị encoder bắt đầu
    en_begin_left = readEncoder(LEFT);
    en_begin_right = readEncoder(RIGHT);
  }
  Serial.println("OK");
